...
2.  **Root Directory**: Click `Edit` and select `frontend`.
    *   *Why?* The Next.js application lives inside the `frontend` folder, not at the root.
...
*   **Error: "The specified Root Directory does not exist"**:
    *   You likely typed the name wrong. Ensure it is exactly `frontend`.
*   **Error: "package.json not found"**:
    *   You forgot to set the Root Directory to `frontend`. Vercel is looking in the root folder where only the Python config exists.
*   **Error: "Module not found" or build failure**:
    *   Ensure `npm install` runs successfully.
    *   Check if you are importing files from outside `frontend/`. Next.js doesn't like importing files above the project root.
...
Check `frontend/src/lib/api.ts` if you need to debug connections.
